        ...
        mapper = [ordered]@{
            ...
            columnValuesToChange = @(
                [ordered]@{
                    ...
                    columnValue = [ordered]@{
                        date = '{{formatDate(replace(toString(42.data); /^[\s\S]*?"start_time":"([^"]+)"[\s\S]*$/; $1); "YYYY-MM-DDTHH:mm:ss"; "Asia/Jerusalem")}}'
                        includeTime = $true
                    }
                },
                [ordered]@{
...
$startTime = 'replace(toString(42.data); /^[\s\S]*?"start_time":"([^"]+)"[\s\S]*$/; $1)'
