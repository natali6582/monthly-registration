        ...
        mapper = [ordered]@{
            ...
            columnValuesToChange = @(
                [ordered]@{
                    ...
                    columnValue = [ordered]@{
                        date = '{{formatDate(42.collection[1].start_time; "YYYY-MM-DDTHH:mm:ss"; "Asia/Jerusalem")}}'
                        includeTime = $true
                    }
                },
                [ordered]@{
...
$eventRange = Copy-Module -Id 31
$eventRange.id = 42
$eventRange.mapper.qs = @(
    [ordered]@{
        key = 'event_type'
        value = 'https://api.calendly.com/event_types/{{41.data.uuid}}'
    },
    [ordered]@{
        key = 'start_time'
        value = '{{41.data.start_date}}T00:00:00Z'
    },
    [ordered]@{
        key = 'end_time'
        value = '{{formatDate(addDays(41.data.end_date; 1); "YYYY-MM-DDT00:00:00[Z]"; "UTC")}}'
    }
)
$eventRange.metadata.designer.x = 1200
$eventRange.metadata.designer.y = 0
$eventRange.metadata.designer.name = 'Calendly API - קריאת תאריך ושעת ההדרכה'
...
$startTime = '42.collection[1].start_time'
