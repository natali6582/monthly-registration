        ...
        mapper = [ordered]@{
            ...
            columnValuesToChange = @(
                [ordered]@{
                    ...
                    columnValue = [ordered]@{
                        date = '{{formatDate(ifempty(31.collection[1].start_time; ifempty(33.collection[1].start_time; ifempty(34.collection[1].start_time; ifempty(35.collection[1].start_time; ifempty(36.collection[1].start_time; 37.collection[1].start_time))))); "YYYY-MM-DDTHH:mm:ss"; "Asia/Jerusalem")}}'
                        includeTime = $true
                    }
                },
                [ordered]@{
...
$availability0To7 = Copy-Module -Id 31
$availability7To14 = Copy-Module -Id 33
$availability14To21 = Copy-Module -Id 34
$availability21To28 = Copy-Module -Id 35
$availability28To35 = Copy-Module -Id 36
$availability35To42 = Copy-Module -Id 37
...
$availabilityX = 1200
foreach ($availabilityModule in $availabilityModules) {
    $availabilityModule.mapper.qs[0].value = 'https://api.calendly.com/event_types/{{41.data.uuid}}'
    $availabilityModule.metadata.designer.x = $availabilityX
    $availabilityModule.metadata.designer.y = 0
    $availabilityModule.metadata.designer.name = "Calendly API - זמינות $($availabilityModule.id)"
    $availabilityX += 300
}

$createItem.metadata.designer.x = 3000
...
$updateDates = New-MondayChangeDatesModule -Id 44 -X 3300 -Name 'Monday - עדכון תאריך ההדרכה ותאריך ההרשמה'
$createdItem = New-MondayGetItemModule -Id 43 -ItemId '{{30.id}}' -X 3600 -Name 'Monday - קריאת כתובת המייל מהפריט שנוצר'

$startTime = 'ifempty(31.collection[1].start_time; ifempty(33.collection[1].start_time; ifempty(34.collection[1].start_time; ifempty(35.collection[1].start_time; ifempty(36.collection[1].start_time; 37.collection[1].start_time)))))'
...
$emailHtmlVariable.metadata.designer.x = 3900
...
$sendEmail.metadata.designer.x = 4200
