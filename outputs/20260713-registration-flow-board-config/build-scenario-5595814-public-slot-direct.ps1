        ...
        mapper = [ordered]@{
            ...
            columnValuesToChange = @(
                [ordered]@{
                    ...
                    columnValue = [ordered]@{
                        date = '{{formatDate(42.data.days[1].spots[1].start_time; "YYYY-MM-DDTHH:mm:ss"; "Asia/Jerusalem")}}'
                        includeTime = $true
                    }
                },
                [ordered]@{
...
$schedulingLink = New-HttpGetModule -Id 40 -X 600 -Name 'Calendly - קריאת הקישור הפעיל' -Url ('https://calendly.com/api/booking/scheduling_links/{{get(split(' + $activeCalendlyLink + '; "/"); 4)}}')
...
$eventRangeUrl = 'https://calendly.com/api/booking/event_types/{{41.data.uuid}}/calendar/range?timezone=Asia%2FJerusalem&diagnostics=false&range_start={{41.data.start_date}}&range_end={{41.data.end_date}}&scheduling_link_uuid={{40.data.uid}}'
$eventRange = New-HttpGetModule -Id 42 -X 1200 -Name 'Calendly - קריאת מועד ההדרכה שנקבע' -Url $eventRangeUrl
$eventRange.mapper.parseResponse = $true
$schedulingLink.parameters.handleErrors = $true
$eventType.parameters.handleErrors = $true
$eventRange.parameters.handleErrors = $true
